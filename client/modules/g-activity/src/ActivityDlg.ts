 ...
 */
module g_activity{
    export class ActivityDlg extends mo.gui.Dlg{
        ...
        _childrenCreated(){
            ...
            for(var i=0; i<self._allExActivitys.length; ++i){
                ...
                if(type == gc.c_prop.activityTypeKey.sevenLogin){
                    ...
                    self._comps.push(ActivityMysteryShop.create().setData({tray: self.container}).show());
                }else if(type==gc.c_prop.activityTypeKey.singleCharge){
                    self._comps.push(ActivitySingleRchg.create().setData({tray: self.container}).show());
                }
            }
    ...
    egret.Boot.once(egret.Boot.AFTER_CONFIG, function(){
        ...
        moduleCfgItem.targetClass = ActivityNewCenter;
